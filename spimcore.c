...
//Match the MEMSIZE defined before
#define MEM(addr) (Mem[addr >> 2])

//the reason that Reg[REGSIZE + 4]???
...
//Define function for finding register through register name
#define NREG(name) (*Nreg(name))

//What does these two arrays mean?
//Used to check data flow
...
//Implementation of Nreg, get register through name, ex: $s1 from RegName[13] -> reg[13]
...
//input format: spimcore.exe input_file -r
